//
//  RichTextEditorNonEditable.swift
//  BuzzyMail
//
...
public protocol RichTextEditorNonEditableDelegate: class {
...
public class RichTextEditorNonEditable: UIView, WKScriptMessageHandler, WKNavigationDelegate, UIScrollViewDelegate {
    ...
    public weak var delegate: RichTextEditorNonEditableDelegate?
    public var height: CGFloat = RichTextEditorNonEditable.defaultHeight
...
//    }
    ...
    public required init?(coder aDecoder: NSCoder) {
        ...
        guard let bundlePath = Bundle(for: type(of: self)).path(forResource: "Resources", ofType: "bundle"),
            ...
            let scriptPath = bundle.path(forResource: "RichTextEditor", ofType: "js"),
            let scriptContent = try? String(contentsOfFile: scriptPath, encoding: String.Encoding.utf8),
            let htmlPath = bundle.path(forResource: "RichTextEditorNonEditable", ofType: "html"),
            let html = try? String(contentsOfFile: htmlPath, encoding: String.Encoding.utf8)
            else { fatalError("Unable to find javscript/html for text editor") }
        ...
        [RichTextEditorNonEditable.textDidChange, RichTextEditorNonEditable.heightDidChange].forEach {
            configuration.userContentController.add(WeakScriptMessageHandler(delegate: self), name: $0)
        }
    ...
    public func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        switch message.name {
        case RichTextEditorNonEditable.textDidChange:
            guard let body = message.body as? String else { return }
            placeholderLabel.isHidden = !body.htmlToPlainText.isEmpty
            delegate?.textDidChange(text: body)
        case RichTextEditorNonEditable.heightDidChange:
            guard let height = message.body as? CGFloat else { return }
            self.height = height > RichTextEditorNonEditable.defaultHeight ? height + 30 : RichTextEditorNonEditable.defaultHeight
